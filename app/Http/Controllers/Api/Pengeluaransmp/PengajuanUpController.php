...
namespace App\Http\Controllers\Api\Pengeluaransmp;
...
class PengajuanUpController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
                if (!$kode) {
                    DB::select('call pengajuanupbendsmp(@nomor)');
                    $nomor = DB::table('counter')->select('pengajuanupbendsmp')->first();
                    $semester = 'S1';
                    $kode = FormatingHelper::notrans($nomor->pengajuanupbendsmp, 'UP', $semester,'SMP');
                }
